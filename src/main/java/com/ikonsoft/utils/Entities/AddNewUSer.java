...
/* 24:40 */       User user = new User(request.getParameter("name"), request.getParameter("Tel"), request.getParameter("email"), request.getParameter("password"));
/* 25:41 */       UserDBConnection db = new UserDBConnection();
/* 26:42 */       db.AddUser(user);
/* 27:43 */     
...
ArrayList<User> users1 = db.GetAdmin();
String admintel="";
String adminpass="";
/* 31:47 */   
for (User i : users1) {
    admintel+=i.getTel();
    adminpass+=i.getPassword();
/* 33:   */       }
out.print("u will be redirect now for message sent");
String url="http://watsapp.ofertasbooking.net/WhatsAPI-Official-master/tests/whatsapp.php?username="+admintel+"&password="+adminpass +
"&phones="+phones;
